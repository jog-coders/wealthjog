...
function DarkTip({ active, payload, label }) {
  ...
  return (
    <div style={{ background: 'var(--bg-main)', border: '1px solid var(--border)', borderRadius: 10, padding: '8px 14px', fontSize: 12 }}>
      {label && <p style={{ margin: '0 0 4px', fontWeight: 600, color: 'var(--text-1)' }}>{label}</p>}
      {payload.filter(e => e.dataKey !== 'base').map((e, i) => (
        <div key={i} style={{ display: 'flex', justifyContent: 'space-between', gap: 16 }}>
          <span style={{ color: e.color || 'var(--text-2)' }}>{e.name}:</span>
          <span style={{ fontWeight: 700, color: 'var(--text-1)' }}>{formatCurrency(Math.abs(e.value))}</span>
        </div>
      ))}
    </div>
  );
...
function CashFlowWaterfall({ property }) {
  ...
  return (
    <div style={CARD}>
      <p style={{ margin: '0 0 2px', fontSize: 13, fontWeight: 700, color: 'var(--text-1)' }}>Monthly Cash Flow</p>
      <p style={{ margin: '0 0 14px', fontSize: 10, color: 'var(--text-3)' }}>Where your rent money goes each month</p>
      <div style={{ height: 230 }}>
        <ResponsiveContainer width="100%" height="100%">
          <BarChart data={data} margin={{ top: 8, right: 8, left: 0, bottom: 0 }} barCategoryGap="28%">
            ...
            <YAxis tickFormatter={v => `$${(v/1000).toFixed(0)}k`} tick={{ fill: 'var(--text-3)', fontSize: 10 }} axisLine={false} tickLine={false} width={44} />
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', marginTop: 10, paddingTop: 10, borderTop: '1px solid var(--border-subtle)' }}>
        <span style={{ fontSize: 11, color: 'var(--text-3)' }}>Monthly Net: <b style={{ color: net >= 0 ? '#00D28E' : '#F87171' }}>{net >= 0 ? '+' : ''}{formatCurrency(net)}</b></span>
        <span style={{ fontSize: 11, color: 'var(--text-3)' }}>Annual: <b style={{ color: net >= 0 ? '#00D28E' : '#F87171' }}>{net >= 0 ? '+' : ''}{formatCurrency(net * 12)}</b></span>
      </div>
    </div>
  );
}
...
function ExpenseDonut({ propertyId }) {
  ...
  return (
    <div style={CARD}>
      <p style={{ margin: '0 0 2px', fontSize: 13, fontWeight: 700, color: 'var(--text-1)' }}>Expense Breakdown</p>
      <p style={{ margin: '0 0 14px', fontSize: 10, color: 'var(--text-3)' }}>All ledger expenses by category</p>
      <div style={{ height: 230 }}>
        {loading ? (
          <div style={{ height: '100%', display: 'flex', alignItems: 'center', justifyContent: 'center', color: 'var(--text-3)' }}>Loading…</div>
        ) : data.length === 0 ? (
          <div style={{ height: '100%', display: 'flex', alignItems: 'center', justifyContent: 'center', color: 'var(--text-3)', fontSize: 12, textAlign: 'center' }}>No expense entries yet</div>
        ) : (
          <ResponsiveContainer width="100%" height="100%">
            <PieChart>
              <defs>
              ...
              <Tooltip content={({ active, payload }) => {
                ...
                return (
                  <div style={{ background: 'var(--bg-main)', border: '1px solid var(--border)', borderRadius: 8, padding: '8px 12px', fontSize: 12 }}>
                    <p style={{ margin: 0, color: 'var(--text-1)', fontWeight: 600 }}>{d.name}</p>
                    <p style={{ margin: '4px 0 0', color: '#F87171', fontWeight: 700 }}>{formatCurrency(d.value)}</p>
                    <p style={{ margin: '2px 0 0', color: 'var(--text-3)', fontSize: 10 }}>{total > 0 ? ((d.value / total) * 100).toFixed(1) : 0}%</p>
                  </div>
                );
              }} />
            </PieChart>
      ...
      {data.length > 0 && (
        <div style={{ display: 'flex', flexWrap: 'wrap', gap: '4px 12px', marginTop: 8, paddingTop: 10, borderTop: '1px solid var(--border-subtle)' }}>
...
function EquityDebtStack({ property }) {
  ...
  return (
    <div style={CARD}>
      <p style={{ margin: '0 0 2px', fontSize: 13, fontWeight: 700, color: 'var(--text-1)' }}>Equity vs Debt</p>
      <p style={{ margin: '0 0 16px', fontSize: 10, color: 'var(--text-3)' }}>Your ownership stake at current market value</p>

      <div style={{ textAlign: 'center', marginBottom: 18 }}>
        <p style={{ margin: 0, fontSize: 30, fontWeight: 900, color: '#00D28E', letterSpacing: '-0.03em' }}>{formatCurrency(equity)}</p>
        <p style={{ margin: '3px 0 0', fontSize: 10, color: 'var(--text-3)' }}>Your Equity ({equityPct.toFixed(1)}% owned)</p>
      </div>
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', marginTop: 10 }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
          ...
          <span style={{ fontSize: 10, color: 'var(--text-3)' }}>Mortgage</span>
      ...
      {appreciation !== 0 && (
        <div style={{ marginTop: 14, paddingTop: 12, borderTop: '1px solid var(--border-subtle)', display: 'flex', justifyContent: 'space-between' }}>
          <span style={{ fontSize: 10, color: 'var(--text-3)' }}>Market Value: <b style={{ color: 'var(--text-2)' }}>{formatCurrency(marketValue)}</b></span>
          <span style={{ fontSize: 10, color: 'var(--text-3)' }}>vs Purchase: <b style={{ color: appreciation >= 0 ? '#00D28E' : '#F87171' }}>{appreciation >= 0 ? '+' : ''}{formatCurrency(appreciation)}</b></span>
        </div>
      )}
    </div>
  );
  ...
  return (
    <div style={CARD}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 14 }}>
        <div>
          <p style={{ margin: 0, fontSize: 13, fontWeight: 700, color: 'var(--text-1)' }}>Market Appreciation</p>
          <p style={{ margin: '2px 0 0', fontSize: 10, color: 'var(--text-3)' }}>Property value over time</p>
        </div>
        {purchase > 0 && gained !== 0 && (
          <div style={{ textAlign: 'right' }}>
            ...
            <p style={{ margin: '1px 0 0', fontSize: 9, color: 'var(--text-3)' }}>vs purchase price</p>
          </div>
        )}
      </div>
      <div style={{ height: 240 }}>
        {loading ? (
          ...
          <div style={{ height: '100%', display: 'flex', alignItems: 'center', justifyContent: 'center', color: 'var(--text-3)', fontSize: 12, textAlign: 'center', padding: '0 24px' }}>
          ...
          <ResponsiveContainer width="100%" height="100%">
            <AreaChart data={chartData} margin={{ top: 8, right: 8, left: 0, bottom: 0 }}>
              ...
              <CartesianGrid strokeDasharray="2 4" stroke="rgba(51,65,85,0.35)" vertical={false} />
              <XAxis dataKey="label" tick={{ fill: 'var(--text-3)', fontSize: 9 }} axisLine={false} tickLine={false} />
              <YAxis tickFormatter={v => `$${(v/1000).toFixed(0)}k`} tick={{ fill: 'var(--text-3)', fontSize: 9 }} axisLine={false} tickLine={false} width={44} />
              <Tooltip content={({ active, payload }) => {
                if (!active || !payload?.length) return null;
                const d = payload[0].payload;
                ...
                return (
                  <div style={{ background: 'var(--bg-main)', border: '1px solid var(--border)', borderRadius: 8, padding: '8px 12px', fontSize: 12 }}>
                    <p style={{ margin: 0, color: 'var(--text-3)', fontSize: 10 }}>{d.date}</p>
              ...
              {purchase > 0 && (
                <ReferenceLine y={purchase} stroke="#475569" strokeDasharray="4 4"
                  label={{ value: 'Purchase', position: 'insideTopLeft', fill: 'var(--text-3)', fontSize: 9 }} />
              )}
