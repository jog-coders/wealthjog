  ...
  if (active && payload && payload.length) {
    ...
    return (
      <div style={{ background: 'var(--bg-card)', border: '1px solid var(--border)', borderRadius: 10, padding: '8px 14px', boxShadow: '0 8px 24px rgba(0,0,0,0.5)' }}>
        <p style={{ margin: 0, fontSize: 12, color: 'var(--text-2)' }}>{name}</p>
        <p style={{ margin: '3px 0 0', fontSize: 14, fontWeight: 700, color: 'var(--text-1)' }}>{formatCurrency(value)}</p>
      </div>
    );
  }
  return null;
...
export default function NetWorthSnapshot() {
  ...
  return (
    <div style={{ background: 'var(--bg-card)', border: '1px solid var(--border)', borderRadius: 16, padding: '20px 24px', boxShadow: '0 0 24px rgba(0,210,142,0.08)' }}>
      <h3 style={{ margin: '0 0 16px', fontSize: 15, fontWeight: 700, color: 'var(--text-1)' }}>Net Worth Snapshot</h3>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: 10, marginBottom: 20 }}>
        <div style={{ textAlign: 'center', background: 'rgba(0,210,142,0.08)', border: '1px solid rgba(0,210,142,0.15)', borderRadius: 10, padding: '10px 6px' }}>
          ...
          <p style={{ fontSize: 13, fontWeight: 700, color: 'var(--text-1)', margin: '4px 0 0' }}>{formatCurrency(totalAssets)}</p>
        </div>
        <div style={{ textAlign: 'center', background: 'rgba(239,68,68,0.08)', border: '1px solid rgba(239,68,68,0.15)', borderRadius: 10, padding: '10px 6px' }}>
          ...
          <p style={{ fontSize: 13, fontWeight: 700, color: 'var(--text-1)', margin: '4px 0 0' }}>{formatCurrency(totalLiabilities)}</p>
        </div>
        <div style={{ textAlign: 'center', background: netWorth >= 0 ? 'rgba(0,210,142,0.08)' : 'rgba(239,68,68,0.08)', border: `1px solid ${netWorth >= 0 ? 'rgba(0,210,142,0.15)' : 'rgba(239,68,68,0.15)'}`, borderRadius: 10, padding: '10px 6px' }}>
          ...
          <p style={{ fontSize: 13, fontWeight: 700, color: 'var(--text-1)', margin: '4px 0 0' }}>{formatCurrency(netWorth)}</p>
        </div>
      </div>
      ...
      <div style={{ height: 240 }}>
        {isEmpty ? (
          <div style={{ height: '100%', display: 'flex', alignItems: 'center', justifyContent: 'center', color: 'var(--text-3)', fontSize: 13 }}>No data yet</div>
        ) : (
