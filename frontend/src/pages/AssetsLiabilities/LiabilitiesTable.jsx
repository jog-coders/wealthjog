...
export default function LiabilitiesTable() {
  ...
  const [type, setType] = useState('other_liability');
  ...
  useEffect(() => {
    get('/api/settings/lookup-values?domain=liability_type').then(({ data }) => {
      if (data && data.length > 0) setLiabilityTypes(data);
    });
  }, [get]);

  const resetForm = () => {
    setName('');
    setType('other_liability');
    setInstitution('');
    setDate(new Date().toISOString().split('T')[0]);
    setAmount(0);
              ...
              {currentLiabilities.map((item) => (
                <Fragment key={item.id}>
                  <tr className="hover:bg-gray-50 transition-colors">
                    ...
                    <td className="whitespace-nowrap px-3 py-4 text-sm text-gray-500">{liabilityTypes.find(t => t.value === item.type)?.label || item.type}</td>
